...
class TranslationImporter @Inject constructor(
    ...
    /**
     * Ensure a `books` row exists for every book imported. The prepopulated
     * database only seeds `books` for the bundled translations (kjv/web/asv/
     * bbe/ylt/dra/geneva); a runtime-imported translation such as NKJV has
     * verses but no book list, so the UI cannot show its books. Copy canonical
     * book metadata from an already-present translation — names and chapter
     * counts are translation-independent — and `INSERT OR IGNORE` so rows that
     * already exist (e.g. BBE) are left untouched.
     */
